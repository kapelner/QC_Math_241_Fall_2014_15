...
\title{MATH 241 Fall 2014 Homework \#11}
...
\date{Due 11:59PM in my office, Thurs Dec 4, 2014 \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
Reading is still \textit{required}. For this homework set, circle back to Chapter 2 and reread it all, even the parts on moment generating functions which you now understand!! Then read chapter 3 as well about topics we have covered.
...
\easysubproblem Using your answer from part (g), write the first five terms of $\expe{ e^{tX} }$ using the property of expectation being a linear operator (\ie $\expe{X+Y} = \expe{X} + \expe{Y}$ valid always) and then write $+ ~\expe{\ldots}$ afterwards.  \spc{2}

\easysubproblem You have written the first five terms of the infinite series representation of $M_X(t)$, the mgf for a r.v. $X$ in the previous problem. Now find $M'_X(t)$, the derivative of the mgf with respect to $t$, the dummy variable in the $t$ domain and then write $+~ \ldots$ afterwards. Remember, anything that's a function of the r.v. $X$ is a constant when differentiating with respect to $t$.  \spc{2}
...
\hardsubproblem Using the fact that $\var{X} = \expe{X^2} - \musq$, find the variance of $T$, a binomial r.v. using the mgf to find its second moment and assuming (by this point) that you know that $\mu = np$. \spc{4}



\hardsubproblem Show that the sum of two independent r.v.'s $T_1 \sim \binomial{n_1}{p}$ and $T_2 \sim \binomial{n_2}{p}$ is itself a binomial and find its parameters. Does this make sense? \spc{4}


\easysubproblem Let $X_1, \ldots, X_n \iid \geometric{p}$. Assume that the mgf for the geometric is:

\beqn
M_{X_i}(t) = \frac{pe^t}{1-(1-p) e^t}
\eeqn

Let $T = X_1 + \ldots + X_n$. We know from our study of discrete r.v.'s that $T \sim \negbin{r}{p}$ which was illustrated conceptually. However, this was never proven. We will prove it here. Using the fact that:
...
\intermediatesubproblem Show that the sum of two independent r.v.'s $T_1 \sim \negbin{r_1}{p}$ and $T_2 \sim \negbin{r_2}{p}$ is itself a negative binomial and find its parameters. Why does this make sense?  \spc{7}
...
Show that the sum of two independent r.v.'s $X_1 \sim \poisson{\lambda_1}$ and $X_2 \sim \poisson{\lambda_2}$ is itself a Poisson r.v. and find its parameter. \spc{4}

\intermediatesubproblem You have shown previously that the mgf for the r.v. $X \sim \binomial{n}{p}$ was $M_X(t) = \tothepow{1-p+pe^t}{n}$. In the previous problem you were given the mgf for the r.v. $X \sim \poisson{\lambda}$ was $M_X(t) = e^{\lambda (e^t - 1)}$. You know that the Poisson distribution is the limit of the Binomial distribution with $n \rightarrow \infty$ and the rate parameter pinned $\lambda = np$. Prove that the Poisson's mgf is this limit of the Binomial's mgf. Really not hard: just some algebraic manipulations.  \spc{4}

\intermediatesubproblem We proved in class that if $Z \sim \stdnormnot$ then $M_Z(t) = e^{t^2/2}$. We also showed that if $X = \sigma Z + \mu$, then $X \sim \normnot{\mu}{\sigsq}$. Show that the mgf of $X$ is $M_X(t) = e^{\mu t + \sigsq t^2 /2}$ using what you learned in 1(t) and 1(u) and the class notes.  \spc{4}
...
Demonstrate that the sum of $X_1, \ldots, X_n \iid \exponential{\lambda}$ is an Erlang r.v. with parameters $n$ and $\lambda$, the continuous analogue of the Negative Binomial r.v.  \spc{10}
...
Prove that the mgf for a standard Cauchy r.v. does not exist. This is really not hard but it looks menacing.  \spc{8}
...
If it's \qu{standardized,} what is the mean and standard error?  \spc{2}

\intermediatesubproblem Why is the above quantity called a \qu{dimensionless number?}  \spc{4}

\intermediatesubproblem We now let $Z_i = (X_i - \mu)/\sigma$. Show that $C_n = \sqrt{n}\Zbar$ is equivalent to $C_n$ (the standardized $\Xbar$) from part (d). \spc{4}
...
\easysubproblem Show that $\sqrt{n}\Zbar = \frac{Z_1}{\sqrt{n}} + \frac{Z_2}{\sqrt{n}} + \ldots + \frac{Z_n}{\sqrt{n}}$. This is trivial algebra once you use the definition of $\Zbar := \overn{Z_1 + \ldots + Z_n}$.  \spc{2}
...
\easysubproblem Write $M_{\frac{Z}{\sqrt{n}}}(t)$ using \qu{little-o} notation using what you showed in the previous problem.  \spc{2}
...
I'm sure the answer is on Google somewhere!   \spc{15}
...
has the same form as a standard normal mgf. This mgf is found in problem 2(k). Thus, since it has the same fingerprint, it must have the standard normal distribution! QED. \spc{6}

\easysubproblem Congratulations: you have proved the most basic central limit theorem, the crown jewel of Math 241! Write a smiley face below above the crown. \spc{6}
...
\begin{enumerate}
\easysubproblem Image $X \sim \bernoulli{\half}$. Run the following code in \texttt{R} and print out (in black and white) the result:
...
Does this look like the PMF of $X$? \spc{2}
...
This will look at the estimated PDF for $C_n$ for $n = 10, 100, 1000, 5000, 10000, 50000$. At what $n$ value does it appear that $C_n$ converged to a $\stdnormnot$? Pay attention to gaps of whitespace in these plots. They mean there's no support there! The standard normal has support everywhere. If it never converges, write \qu{never.} \spc{1}
...
This will look at the estimated PDF for $C_n$ for $n = 10, 100, 1000, 5000, 10000, 50000$. At what $n$ value does it appear that $C_n$ converged to a $\stdnormnot$? Pay attention to gaps of whitespace in these plots. They mean there's no support there! The standard normal has support everywhere. If it never converges, write \qu{never.} \spc{2}
...
This will look at the estimated PDF for $C_n$ for $n = 10, 100, 1000, 5000$. At what $n$ value does it appear that $C_n$ converged to a $\stdnormnot$? Pay attention to gaps of whitespace in these plots. They mean there's no support there! The standard normal has support everywhere. If it never converges, write \qu{never.} \spc{5}
...
\intermediatesubproblem Why did this converge immediately? See question 2(m) for the answer. \spc{3}
...
I will consider this to be called the \qu{bathtub function.} This is not a real brand name distribution. I just made it up and it won't be on the test. Write about why you think I called it the \qu{bathtub function.} Does this look like a bell curve at all?? \spc{3}
...
This will look at the estimated PDF for $C_n$ for $n = 2, 5, 10, 50, 100, 1000$. At what $n$ value does it appear that $C_n$ converged to a $\stdnormnot$? Pay attention to gaps of whitespace in these plots. They mean there's no support there! The standard normal has support everywhere. If it never converges, write \qu{never.} \spc{2}

\easysubproblem Let's sum up what we've learned in this problem. The central limit states as $n \rightarrow \infty$ then $C_n$ becomes a standard normal. Does it become a standard normal at different rates depending on the distribution of the r.v. being sampled?
