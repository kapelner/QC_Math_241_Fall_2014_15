...
\extracreditsubproblem Prove or disprove that you can design a coin flipping system which yields an arbitrary $p \in \rationals \cap \zeroonecl$. Disclaimer: I tried this problem for a half hour and could not solve it. \spc{4.5}
...
\begin{enumerate}
\easysubproblem In the previous problem, you used coins to design the random variable $X \sim \bernoulli{\half}$ using the $\Omega$ from a coin flip. There are other ways to create $X$. For instance: roll a die and map outcomes 1,2,3 to 0 and outcomes 4,5,6 to 1. This works because 
...
\intermediatesubproblem We talked about in class how the sample space no longer needs to be considered once the random variable is described. Why? Use your answer to (a) to inspire this answer. Write it \textit{in English} below. \spc{5}
...
\begin{enumerate}
\easysubproblem Draw the PMF for $X \sim \bernoulli{\half}$.  \spc{4}
...
\easysubproblem Using the r.v. from the previous question, what is $\prob{X \in (3,9)}$? \spc{3}

\easysubproblem Take a r.v. $X$ with $\support{X} = \zeroonecl$. Is this a \qu{discrete r.v.?} Explain. \spc{1.5}
...
but we did not put its PMF on the board. Write $f(x)$ for $X \sim \bernoulli{p}$ that is only valid on $\support{X}$. \spc{3}


\extracreditsubproblem Write a PMF for $X \sim \bernoulli{p}$ that is valid always $\forall x \in \reals$.  \spc{3}

\hardsubproblem Sometimes knowing the $\Omega$ matters a little bit. Let's say $X_1 \sim \bernoulli{\half}$ is generated from one coin and $X_2 \sim \bernoulli{\half}$ is generated from another coin independently tossed. Create a new r.v. $T = X_1 + X_2$. Describe its PMF using the $\sim$ notation like in the previous problem. \spc{8}

\hardsubproblem It may seem to you that we use $F$ for CDF and $f$ for PMF because there's a connection to derivatives and anti-derivatives. We are going to see if this is true here? Consider the PMF we discussed for $X \sim \bernoulli{\half}$. Does $\myint{x}{}{}{f(x)} = F(x) + C$ where the constant $C \in \reals$? Explain. \spc{4}

\hardsubproblem How about the opposite? Consider the CDF we discussed for $X \sim \bernoulli{\half}$. Does $\text{d} / \text{d}x[F(x)] = f(x)$? Explain. \spc{4}

\end{enumerate}

\problem We now combine the concept of recursing trees with random variables. This r.v. is called the geometric r.v. and will not be tested on the upcoming midterm. I do expect you to be able to do this type of reasoning on the midterm though.

Imagine you play a game. You flip a coin with the probability of heads is not necessarily fair. Define $p := \prob{\braces{H}}$. If you get heads, you stop. If you get tails, you keep flipping. 

\begin{enumerate}
\easysubproblem How many flips of the coins can you potentially flip? Describe the set of number of flips that could occur. Use the notation from lecture 1. \spc{2}

\easysubproblem What is the probability you flip three times? That is two tails and a head.\spc{3}

\intermediatesubproblem What is the probability you flip $x$ times? \spc{3}

\easysubproblem Let $X$ be the r.v. that models the number of flips. What is the $\support{X}$? Is it finite? \spc{2}

\easysubproblem What is the parameter space for $p$? \spc{3}

\hardsubproblem Find the PMF for $X$. This PMF only has to be valid $\forall x \in  \support{X}$. \spc{4}


\hardsubproblem Find the CDF for $X$. \spc{4}

\intermediatesubproblem In class we spoke about the two properties of PMFs. The first property is that $f(x) \in \zeroonecl$. Show that this is true $\forall x \in  \support{X}$ for this r.v.  \spc{5}

\hardsubproblem In class we spoke about the two properties of PMFs. The second property is that

\beqn
\sum_{x \in  \support{X}} f(x) = 1
\eeqn

which is similar to the concept in probability of $\prob{\Omega} = 1$. Show that this holds for this r.v. \spc{6}

\easysubproblem In class we spoke a lot about \qu{successes} and \qu{failures.} In English, explain the resulting outcomes of this random variable using the words \qu{successes} and \qu{failures.}
\end{enumerate}
